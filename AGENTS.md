...
- **État actuel** : `houetor-connect` v2.7.0 (ops structurelles move/duplicate/wrap/unwrap ; structural 42/42, V3 32/32, rétention 9/9, transform 21/21, tier policy 11/11) ; `houetor-mcp/` v2.7.0 (42/42 unitaires, 52/52 intégration, 41/41 scénarios) ; zip 2.7.0 reconstruit (niveau unique) ; stable tag readme.txt 2.7.0 ; **branche `opencode-learning` = `main` = `08c6199` (clôture faite, tout poussé)**.
...
- **Fils ouverts (actions utilisateur)** : connexion du site Fix Day au dashboard HOUETOR (connected_sites) ; merge/rollout `mcp-block-crud-2.7.0` ; audit `houetor-selfhare` (NE PAS y toucher sans validation) ; évolutions roadmap restantes (compte agent WP moindre privilège, rate limit rewrites séparé, PHPUnit) ; `ghjk.py` (D non commité dans houetor) + 3 `probe-*.mjs` lab untracked.
