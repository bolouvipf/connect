...
export function translateError(status: number, data: any, fallback: string): TranslatedError {
  ...
  if (status === 401 || status === 403) {
    return {
      ...
      code: 'unauthorized',
      message:
        `Jeton plugin invalide (${raw}). Vérifiez HOUETOR_TOKEN : le token lab ` +
        `("hwc_token" du site de test) doit être celui retourné par /pages avec le header X-Houetor-Token attendu.`,
    }
  }
  ...
  if (status === 409 && code === 'error_conflict') {
    return {
      ...
      message:
        ...
        `(${raw}). Re-lisez la page (get_page_blocks ou /pages) pour obtenir un content_md5/expected_hash à jour, puis réessayez.`,
    }
  }
  ...
  if (status === 429 && code === 'rate_limited') {
    return {
      ...
      message:
        ...
        `(${raw}). Patientez ~60 s ou regroupez vos écritures (le batch update_blocks arrive en 2.4.0).`,
    }
  }
