# Portage `app/mcp/` — CRUD bloc v2.6.0 (Phase 4 mission)
...
1. **`error-translator.ts`** (nouveau, identique au miroir 2.6.0) — 401/403, 409 `error_conflict`, 429 `rate_limited`, 404 `anchor_not_found`/`block_not_found`, **400 `block_legacy`** (tier policy : conseil de remplacement du bloc) traduits en **conseils actionnables** pour l'agent.
2. **`tools.ts`** — 7 nouveaux tools (profils tous clients) :
...
3. **`dispatch.ts`** — helpers `resolveSite` + `pluginRequest` (appels `houetor/v1` avec token `connected_sites` et erreurs traduites), 7 nouvelles méthodes, `dry_run`/`expected_hash` sur inject/uninject.
...
- `npx tsc --noEmit` (tsconfig ci-contre, types réels du repo prod) : **0 erreur** (2.6.0)
- Miroir `houetor-mcp/` v2.6.0 : unitaires 30/30, intégration 35/35, scénarios 29/29 (preuve du protocole, tier policy incluse)
- Diff `original/` → `src/` : dispatch.ts +292/−41, tools.ts +82, error-translator.ts +63
