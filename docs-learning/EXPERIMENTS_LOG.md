...
## Exp 041 - SECTION 29, Domino 3 : assets + readme.txt soumission WordPress.org (08/08/2026)

**Contexte** : Domino 3 (J35-J55) du brief Section 29 = preparer les assets et le readme.txt pour la soumission WordPress.org du plugin houetor-connect 2.9.0. Brand kit pilote : `public/HOUETOR_BrandKit_2026.pdf` (5 pages, extrait par pymupdf - le sous-agent visionneur ne renvoie rien, pas de support image dans le modele principal) -> palette #0D1F1A/#1A3D2E/#2ECC8A/#F0EDE6/#7A9E8E, logos ellipse horizontale, Space Grotesk Bold + DM Sans. Les 4 candidats Canva generes etaient tous mauvais (rejet utilisateur "tous sont nulles, attends que je gere ca") -> **l'utilisateur a fourni les visuels finaux en PNG** dans Downloads, traitement script python/Pillow uniquement.

**1. 5 assets dans houetor-connect/assets/** (poids total 252.9 KB, < 1MB) :
- `icon-128x128.png` 128x128, 5.0 KB (<100KB) : source "Capture d'ecran 2026-07-14 181725.png" (241x178, Image 1 du chat - illisible par le modele, traitee en aveugle) -> redimensionnee LANCZOS, posee centree sur canvas blanc 128x128 (padding >= 10%).
- `banner-772x250.png` 772x250, 97 KB (<150KB) : source `banner_772x250_houetor_precise.png` (1910x823, ratio 2.32 vs cible 3.09) -> recadrage vertical centre + resize LANCZOS + **quantification 128 couleurs** (MEDIANCUT+FLOYDSTEINBERG : 234KB -> 97KB ; 256c=130KB, 64c=97KB mais 128c retenu pour qualite).
- `screenshot-1.png` 680x500, 68 KB / `screenshot-2.png` 680x500, 82 KB / `screenshot-3.png` 680x500, 60 KB (<200KB chacun) : sources screenshot_1/2/3_wp_content.png (2760x2040, ratio 1.35 vs cible 1.36) -> crop centre + resize.
- Scripts jette-apres : `C:\Users\Kimsh\AppData\Local\Temp\opencode\make-assets.py`, `reprocess-banner.py`.

**2. readme.txt reecrit** (structure stricte WordPress.org, UTF-8 sans BOM) : header `=== HOUETOR Connect ===` (Contributors BOLOUVI Pierre Florent + houetor, Tags automation/ai/content/blocks/gutenberg, Requires at least 5.9, Tested up to 6.6, Requires PHP 7.4, Stable tag 2.9.0, License GPL-2.0-or-later) ; tagline 62 chars (< 160) ; Description (agent IA langage naturel) ; Fonctionnalites 5 ; Cas d'usage 4 ; Comment ca fonctionne 4 etapes ; Prerequis ; **Plan payant optionnel** (23k FCFA/mois, Claude, sans lien de paiement direct - conforme) ; Installation 5 etapes ; FAQ 5 ; Changelog 2.9.0 -> 2.1.0 ; Support ; Liens. Remplace l'ancien readme (2.8.0, contributeur unique, pont HWT). Correctif : coquille "Autom ise" -> "Automatise".
